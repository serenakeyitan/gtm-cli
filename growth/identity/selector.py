...
from growth.identity.manager import Identity, IdentityManager
    ...
    if not healthy:
        if accounts:
            raise ValueError(
                ...
                f"Run: growth auth {platform}"
            )
        raise ValueError(f"No {platform} account connected. Run: growth auth {platform}")
...
def handle_account_failure(
    ...
    if not alternatives:
        ...
        console.print(f"  Fix this account: [bold]growth auth {platform}[/bold]")
        return None
    ...
    if action == "stop":
        console.print(f"  Stopped. Fix the account: [bold]growth auth {platform}[/bold]")
        return None
